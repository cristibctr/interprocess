...
macro_rules! derive_asraw {
    ...
    ($({$($forcl:tt)*})? $ty:ty, wasix) => {
        derive_asraw!(
            ...
            $({$($forcl)*})? $ty,
            RawFd, AsRawFd, as_raw_fd,
            AsFd, as_fd, ignored, wasix);
    };
    (@impl
        $({$($forcl:tt)*})?
        $ty:ty,
        $hty:ident, $trt:ident, $mtd:ident,
        $strt:ident, $smtd:ident, $_:ident, wasix) => {
        #[cfg(target_vendor = "wasmer")]
        impl $(<$($forcl)*>)? ::std::os::wasi::io::$trt for $ty {
            #[inline]
            fn $mtd(&self) -> ::std::os::wasi::io::$hty {
                let h = ::std::os::wasi::io::$strt::$smtd(self);
                ::std::os::wasi::io::$trt::$mtd(&h)
            }
        }
    };
    ($({$($forcl:tt)*})? $ty:ty) => {
...
macro_rules! derive_intoraw {
    ...
    ($({$($forcl:tt)*})? $ty:ty, wasix) => {
        derive_intoraw!(
            ...
            $({$($forcl)*})? $ty,
            RawFd, OwnedFd,
            IntoRawFd, into_raw_fd, ignored, wasix);
    };
    (@impl
        $({$($forcl:tt)*})?
        $ty:ty,
        $hty:ident, $ohty:ident,
        $trt:ident, $mtd:ident, $_:ident, wasix) => {
        #[cfg(target_vendor = "wasmer")]
        impl $(<$($forcl)*>)? ::std::os::wasi::io::$trt for $ty {
            #[inline]
            fn $mtd(self) -> ::std::os::wasi::io::$hty {
                let h = <std::os::wasi::io::$ohty as ::std::convert::From<_>>::from(self);
                ::std::os::wasi::io::$trt::$mtd(h)
            }
        }
    };
...
macro_rules! derive_asintoraw {
    ...
    ($({$($forcl:tt)*})? $ty:ty) => {
        ...
        derive_asintoraw!($({$($forcl)*})? $ty, wasix);
    };
}
...
macro_rules! derive_fromraw {
    ...
    ($({$($forcl:tt)*})? $ty:ty, wasix) => {
        derive_fromraw!(
            ...
            $({$($forcl)*})? $ty,
            RawFd, OwnedFd,
            FromRawFd, from_raw_fd, ignored, wasix);
    };
    (@impl
        $({$($forcl:tt)*})?
        $ty:ty,
        ...
        impl $(<$($forcl)*>)? ::std::os::wasi::io::$trt for $ty {
            #[inline]
            unsafe fn $mtd(fd: ::std::os::wasi::io::$hty) -> Self {
                let h: ::std::os::wasi::io::$ohty = unsafe { ::std::os::wasi::io::$trt::$mtd(fd) };
                ::std::convert::From::from(h)
            }
        }
    };
